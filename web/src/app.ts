...
async function refresh() {
  try {
    const rows = [];
    for (let s = 0; s < 4; s++) rows.push(slotRow(await live(s)));
    document.getElementById("slots")!.innerHTML = rows.join("");
  } catch (e) {
    stopPolling();
    renderDisconnected(`lost device: ${(e as Error).message}`);
  }
}
...
async function onStart() { await transport!.send(start()); await sleep(1500); refresh(); }
async function onStop() { await transport!.send(stop()); await sleep(400); refresh(); }
